...
import FormattedDate from '@/components/FormattedDate'; // The component to fix date errors
...
const TableOfContents = ({ contentBlocks }: { contentBlocks?: IContentBlock[] | null }) => {
  ...
  const headings = contentBlocks.filter(block => block.type === 'header' && block.data?.text && [2, 3].includes(block.data.level)).map(block => ({ level: block.data.level, text: block.data.text, slug: (block.data.text ?? '').toLowerCase().replace(/\s+/g, '-').replace(/[^\w-]+/g, '') }));
  if (headings.length === 0) return (<div className="p-4 bg-gray-800 rounded-lg border border-gray-700"><p className="text-lg font-bold text-white mb-3">Table of Contents</p><p className="text-sm text-gray-400">This article has no sections.</p></div>);
  ...
  return (<div className="p-4 bg-gray-800 rounded-lg border border-gray-700"><p className="text-lg font-bold text-white mb-3">Table of Contents</p><ul className="space-y-2">{headings.map((heading, index) => (<li key={index} style={{ marginLeft: `${(heading.level - 2) * 1}rem` }}><a href={`#${heading.slug}`} onClick={(e) => handleScroll(e, heading.slug)} className="text-gray-300 hover:text-red-400 transition-colors text-sm cursor-pointer" dangerouslySetInnerHTML={{ __html: heading.text }} /></li>))}</ul></div>);
};
...
const PostMeta = ({ categories, tags }: { categories?: ICategory[], tags?: ITag[] }) => {
  ...
  return (<div className="space-y-6">{hasCategories && (<div className="p-4 bg-gray-800 rounded-lg border border-gray-700"><p className="text-md font-semibold text-white mb-3 uppercase tracking-wider">Categories</p><div className="flex flex-wrap gap-2">{categories.map(cat => (<Link key={cat._id.toString()} href={`/blog/category/${encodeURIComponent(cat.name.toLowerCase())}`} className="bg-blue-900/50 text-blue-300 text-xs font-medium px-3 py-1 rounded-full hover:bg-blue-900 transition-colors">{cat.name}</Link>))}</div></div>)}{hasTags && (<div className="p-4 bg-gray-800 rounded-lg border border-gray-700"><p className="text-md font-semibold text-white mb-3 uppercase tracking-wider">Tags</p><div className="flex flex-wrap gap-2">{tags.map(tag => (<Link key={tag._id.toString()} href={`/blog/tag/${encodeURIComponent(tag.name.toLowerCase())}`} className="bg-gray-700 text-gray-300 text-xs font-medium px-3 py-1 rounded-full hover:bg-gray-600 transition-colors">#{tag.name}</Link>))}</div></div>)}</div>);
};
    ...
    <div className="prose prose-invert max-w-none prose-p:text-gray-300 prose-headings:text-white prose-a:text-red-500 hover:prose-a:text-red-400 transition-colors">
      {contentBlocks.map((block) => {
        ...
        switch (block.type) {
          ...
          case 'image':
            // FIX: Ensure the image URL is absolute for the Next.js Image component
            const imageUrl = block.data.file.url.startsWith('http') ? block.data.file.url : `${publicDomain}${block.data.file.url}`;
            return <figure key={block.id} className="my-8"><Image src={imageUrl} alt={block.data.caption ?? 'Blog image'} width={800} height={450} className="rounded-lg w-full h-auto object-cover" /><figcaption className="text-center text-sm text-gray-400 mt-2">{block.data.caption}</figcaption></figure>;
          case 'list':
            ...
            return (
              <div key={block.id} className="my-6 overflow-x-auto">
                <table className="w-full text-left border-collapse">
                  {block.data.withHeadings && (<thead><tr className="bg-gray-700/50">{block.data.content[0].map((cellContent: string, cellIndex: number) => (<th key={cellIndex} className="p-3 border border-gray-600 font-semibold" dangerouslySetInnerHTML={{ __html: cellContent }} />))}</tr></thead>)}
...
export default function BlogPostLayout({ post }: { post: IPost }) {
  ...
  // FIX: Ensure the featured image URL is absolute
  const featuredImageUrl = post.featuredImageUrl ? (post.featuredImageUrl.startsWith('http') ? post.featuredImageUrl : `${publicDomain}${post.featuredImageUrl}`) : null;
        ...
        <main className="lg:col-span-6">
          ...
          <article>
            <header className="mb-8">
              ...
              {/* FIX: Using the safe FormattedDate component to prevent hydration errors */}
