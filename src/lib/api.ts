// src/lib/api.ts
import { Match, LeagueGroup, Standing, Player } from "@/data/mockData";
import { isToday, format } from 'date-fns';

// --- INTERFACES for API-Football Data Structures ---
interface ApiOddValue { value: string; odd: string; }
interface ApiBookmaker { id: number; name: string; bets: { id: number; name: string; values: ApiOddValue[] }[]; }
interface ApiOdd { fixture: { id: number }; bookmakers: ApiBookmaker[]; }
interface ApiFixture {
    fixture: { 
        id: number; 
        date: string;
        venue: { name: string | null; city: string | null; };
        status: { long: string; short: any; elapsed: number; }; 
    };
    league: { id: number; name: string; season: number; country: string; logo: string; flag: string | null; };
    teams: { home: { id: number, name: string; logo: string; winner?: boolean }; away: { id: number, name: string; logo: string; winner?: boolean }; };
    goals: { home: number | null; away: number | null; };
    oddsData?: ApiOdd;
}
interface ApiLeague { id: number; name: string; logo: string; }
interface ApiStanding { rank: number; team: { id: number; name: string; logo: string; }; points: number; all: { played: number; win: number; draw: number; loss: number; }; goalsDiff: number; form: string; }
interface ApiPlayer { player: { id: number; name: string; photo: string; }; statistics: [{ goals: { total: number | null }, games: { rating: string | null } }]; }
interface ApiPlayerDetail {
    player: { id: number; name: string; photo: string; };
    statistics: [{ team: { id: number; name: string; logo: string; } }];
}
interface ApiEvent {
  time: { elapsed: number; };
...
export interface MatchDetails extends Match {
  ...
  statistics?: Record<string, { home: string | number; away: string | number; }>;
}

// --- MAPPING FUNCTIONS (UNCHANGED) ---
function mapStatus(status: ApiFixture['fixture']['status']): { status: Match['status'], time?: string } {
...
function mapApiFixtureToMatch(apiFixture: ApiFixture): Match | null {
    ...
    if (apiFixture.oddsData) {
        const bookmaker = apiFixture.oddsData.bookmakers.find(b => b.id === 8) || apiFixture.oddsData.bookmakers[0];
...
export async function fetchDashboardData(): Promise<LeagueGroup[]> {
    if (!API_KEY) { console.error("API Key is missing!"); return []; }
    ...
    try {
        ...
        if (!liveResponse.ok || !todayResponse.ok) { console.error("One or more main API calls failed."); return []; }
    ...
    } catch (error) { console.error("Error fetching dashboard data:", error); return []; }
}
export async function fetchMatchesForClient(date: Date): Promise<LeagueGroup[]> {
    if (!API_KEY) { console.error("Client API Key is missing!"); return []; }
    ...
    try {
        ...
        matches.sort((a, b) => { const order = { 'LIVE': 1, 'HT': 2, 'UPCOMING': 3, 'FT': 4 }; return order[a.status] - order[b.status]; });
        return groupMatchesByLeague(matches);
    } catch(e) { console.error("Error fetching client-side matches:", e); return [] }
}
export async function fetchTopLeagues(): Promise<ApiLeague[]> {
    ...
    } catch (error) { console.error("Failed to fetch top leagues:", error); return []; }
}
export async function fetchStandings(leagueId: string, season: string = "2023"): Promise<Standing[]> {
    ...
    } catch (error) { console.error(`Failed to fetch standings for league ${leagueId}:`, error); return []; }
}
export async function fetchTeamOfTheWeek(leagueId: string = "39", season: string = "2023"): Promise<Player[]> {
    ...
    } catch (error) { console.error("Error in fetchTeamOfTheWeek:", error); return []; }
}

// --- ENHANCED FUNCTION FOR MATCH DETAIL PAGE ---
export async function fetchMatchDetailsById(id: string): Promise<MatchDetails | null> {
    ...
    try {
        ...
        if (statsRes.ok) {
            ...
            // --- ADDED DEBUGGING LOG ---
            console.log(`--- Raw Stats API Response for fixture ${id} ---`, JSON.stringify(statsData, null, 2));

            if (statsData.response && statsData.response.length === 2) {
                const homeStats = statsData.response.find((s: any) => s.team.id === home.id)?.statistics || [];
                const awayStats = statsData.response.find((s: any) => s.team.id === away.id)?.statistics || [];
                const processedStats: Record<string, { home: any; away: any; }> = {};
                homeStats.forEach((stat: ApiStatisticItem) => {
                    processedStats[stat.type] = { home: stat.value ?? 0, away: 0 };
                });
                awayStats.forEach((stat: ApiStatisticItem) => {
                    if (processedStats[stat.type]) {
                        processedStats[stat.type].away = stat.value ?? 0;
                    } else {
                        processedStats[stat.type] = { home: 0, away: stat.value ?? 0 };
                    }
                });
        ...
        console.error(`Error fetching full match details for ID ${id}:`, error);
        return null;
    }
}