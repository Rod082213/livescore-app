...
const EditorJsPreviewRenderer = ({ data }: { data: OutputData }) => {
  ...
  return (<div className="prose prose-invert max-w-none prose-p:text-gray-300 prose-headings:text-white prose-a:text-blue-400">{data.blocks.map((block: any) => { switch (block.type) { case 'header': const { level, text } = block.data; if (!level || !text) return null; if (level === 1) return <h1 key={block.id}>{text}</h1>; if (level === 2) return <h2 key={block.id}>{text}</h2>; if (level === 3) return <h3 key={block.id}>{text}</h3>; return null; case 'paragraph': return <div key={block.id} dangerouslySetInnerHTML={{ __html: block.data.text }} />; case 'image': return (<figure key={block.id}><img src={block.data.file.url} alt={block.data.caption || 'Image'} className="rounded-lg" /><figcaption>{block.data.caption}</figcaption></figure>); case 'list': const ListTag = block.data.style === 'ordered' ? 'ol' : 'ul'; return (<ListTag key={block.id}>{block.data.items.map((item: any, index: number) => { const content = typeof item === 'object' ? item.content : item; return (<li key={index} dangerouslySetInnerHTML={{ __html: content }} />);})}</ListTag>); case 'table': return (<div key={block.id}><table className="w-full"><tbody>{block.data.content.map((row: string[], rIndex: number) => (<tr key={rIndex}>{row.map((cell: string, cIndex: number) => (<td key={cIndex} dangerouslySetInnerHTML={{ __html: cell }} />))}</tr>))}</tbody></table></div>); default: return null; }})}</div>);
};
