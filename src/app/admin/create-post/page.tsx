...
// --- Preview Renderer Component (With the Fix) ---
          ...
          case 'image':
            return (
              <figure key={block.id} className="my-6">
                <img src={block.data.file.url} alt={block.data.caption || 'Content image'} className="rounded-lg w-full h-auto object-cover" />
                {block.data.caption && <figcaption className="text-center text-sm text-gray-400 mt-2">{block.data.caption}</figcaption>}
              </figure>
            );
          
          // --- THIS IS THE FIX FOR [object Object] ---
          case 'list':
            const ListTag = block.data.style === 'ordered' ? 'ol' : 'ul';
            const listStyle = block.data.style === 'ordered' ? 'list-decimal' : 'list-disc';
            return (
              <ListTag key={block.id} className={`${listStyle} pl-5 space-y-2`}>
                {block.data.items.map((item: any, index: number) => {
                  // Check if the item is an object with a 'content' property, or just a string.
...
const CreatePostPage = () => {
    // State Management and Functions... (no changes needed here)
    ...
    const fetchInitialData = useCallback(async () => { setIsPostsLoading(true); try { const [postsRes, catRes, tagRes] = await Promise.all([fetch('/api/posts'), fetch('/api/categories'), fetch('/api/tags')]); if (postsRes.ok) setPosts(await postsRes.json()); if (catRes.ok) setAllCategories((await catRes.json()).map((c: ICategory) => ({ value: c._id.toString(), label: c.name }))); if (tagRes.ok) setAllTags((await tagRes.json()).map((t: ITag) => ({ value: t._id.toString(), label: t.name }))); } catch (error) { setFormError("Failed to load CMS data."); } finally { setIsPostsLoading(false); } }, []);
    ...
    const handleTitleChange = (e: React.ChangeEvent<HTMLInputElement>) => { setTitle(e.target.value); setSlug(e.target.value.toLowerCase().trim().replace(/[^\w\s-]/g, '').replace(/[\s_-]+/g, '-')); };
    const resetForm = () => { setEditingPostId(null); setTitle(''); setSlug(''); setAuthor(''); setDescription(''); setFeaturedImageUrl(''); setContent({ time: Date.now(), blocks: [], version: "2.28.2" }); setSelectedCategories([]); setSelectedTags([]); setFormError(null); window.scrollTo({ top: 0, behavior: 'smooth' }); };
    const handleLoadPostForEdit = (post: IPost) => { setEditingPostId(post._id.toString()); setTitle(post.title); setSlug(post.slug); setAuthor(post.author); setDescription(post.description || ''); setFeaturedImageUrl(post.featuredImageUrl || ''); setContent(post.content as OutputData); const postCategoryIds = (post.categories as any[]).map(c => typeof c === 'string' ? c : c._id.toString()); const postTagIds = (post.tags as any[]).map(t => typeof t === 'string' ? t : t._id.toString()); setSelectedCategories(allCategories.filter(opt => postCategoryIds.includes(opt.value))); setSelectedTags(allTags.filter(opt => postTagIds.includes(opt.value))); window.scrollTo({ top: 0, behavior: 'smooth' }); };
    const handleSubmit = async (e: React.FormEvent) => { e.preventDefault(); setIsSubmitting(true); setFormError(null); if (!content.blocks || content.blocks.length === 0) { setFormError("Content cannot be empty."); setIsSubmitting(false); return; } const body = { title, slug, author, description, featuredImageUrl, content, categories: selectedCategories.map(c => c.value), tags: selectedTags.map(t => t.value) }; const isUpdating = !!editingPostId; const url = isUpdating ? `/api/posts/${editingPostId}` : '/api/posts'; const method = isUpdating ? 'PUT' : 'POST'; try { const response = await fetch(url, { method, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) }); if (response.ok) { alert(`Post successfully ${isUpdating ? 'updated' : 'published'}!`); if (!isUpdating) resetForm(); await fetchInitialData(); } else { const data = await response.json(); setFormError(data.message || 'An error occurred.'); } } catch (error) { setFormError('A network error occurred.'); } finally { setIsSubmitting(false); } };
    const handleDelete = async (postId: string) => { if (!window.confirm("Are you sure?")) return; try { const response = await fetch(`/api/posts/${postId}`, { method: 'DELETE' }); if (response.ok) { alert("Post deleted."); if (editingPostId === postId) resetForm(); await fetchInitialData(); } else { const data = await response.json(); setFormError(data.message || "Failed to delete post."); } } catch(e){ setFormError("Error deleting post."); } };
