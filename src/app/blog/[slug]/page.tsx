...
export async function generateMetadata({ params }: { params: { slug: string } }): Promise<Metadata> {
  ...
  // 1. Determine Title: Prioritize H1 from content, then fall back to the DB title.
  ...
  // 2. Determine Description: Prioritize the dedicated meta description field, then fall back to the first paragraph.
  ...
  // 3. Determine Keywords: Generate from title, or fall back to the post's tags.
  ...
  if (!keywords && post.tags && (post.tags as ITag[]).length > 0) {
    keywords = (post.tags as ITag[]).map(tag => tag.name).join(', ');
  }

  // 4. Define All Other Meta Tags
  ...
  return {
    ...
    keywords: keywords || undefined, // Return undefined if empty to hide the tag
    authors: [{ name: authorName }], // Use the `authors` key with the correct structure
...
const TableOfContents = ({ contentBlocks }: { contentBlocks?: IContentBlock[] | null }) => {
  if (!contentBlocks || !Array.isArray(contentBlocks)) return (<div className="p-4 bg-gray-800 rounded-lg border border-gray-700"><h2 className="text-lg font-bold text-white mb-3">Table of Contents</h2><p className="text-sm text-gray-400">No content available.</p></div>);
