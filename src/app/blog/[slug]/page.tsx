...
export async function generateMetadata({ params }: { params: { slug: string } }): Promise<Metadata> {
  ...
  const safeDescription = firstParagraph ? (firstParagraph.data.text ?? '').replace(/<[^>]*>/g, '').slice(0, 160) : (post.description || 'Read this article on TodayLiveScores.');

  return { title: `${mainTitle} | TodayLiveScores Blog`, description: safeDescription };
}
...
const TableOfContents = ({ contentBlocks }: { contentBlocks?: IContentBlock[] | null }) => {
  ...
  return (
    <div className="p-4 bg-gray-800 rounded-lg border border-gray-700">
      <h2 className="text-lg font-bold text-white mb-3">Table of Contents</h2>
      <ul className="space-y-2">{headings.map((heading, index) => (<li key={index} style={{ marginLeft: `${(heading.level - 1) * 1}rem` }}><a href={`#${heading.slug}`} className="text-gray-300 hover:text-blue-400 transition-colors text-sm">{heading.text}</a></li>))}</ul>
    </div>
  );
};
      ...
      <div className="container mx-auto px-4 py-12">
        <div className="lg:grid lg:grid-cols-12 lg:gap-12">
          ...
          <main className="lg:col-span-6">
            <BackToBlogs />
            <article>
              {post.featuredImageUrl && (<div className="relative w-full h-64 md:h-96 mb-8 rounded-xl overflow-hidden shadow-lg"><Image src={post.featuredImageUrl} alt={post.title} fill className="object-cover" priority /></div>)}
